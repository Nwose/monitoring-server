...
const linkClasses = 'flex items-center gap-2 px-3 py-3 hover:bg-red-400 hover:no-underline active:bg-slate-200 rounded-sm text-base'
const Sidebar = ({collapseNav}) => {
  return (
    <div className={`bg-teal-700 ${!collapseNav ? "w-60" : "w-10"} duration-300 flex flex-col`}>
      <div className={`bg-teal-600 ${!collapseNav ? "p-7" : "p-5 -m-2"} text-white`}>{!collapseNav ? 'MONITORING SERVER' : 'M'}</div>
      <div>
